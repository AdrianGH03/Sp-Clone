...
async function getTokenFromBackend() {
  try {
    const response = await fetch('https://sp-clone-be.netlify.app/api/token'); // Changed the endpoint to the backend route
...
async function fetchPlaylistTracksFromBackend(playlistURL) {
  try {
    ...
    const response = await fetch(`https://sp-clone-be.netlify.app/api/playlist-track-uri?playlistUrl=${encodedURL}`);
    const data = await response.json();
