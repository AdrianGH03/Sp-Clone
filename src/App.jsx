...
async function getTokenFromBackend() {
  try {
    const response = await fetch('https://spotify-clone-backend-empc.onrender.com/api/token'); // Changed the endpoint to the backend route
...
async function fetchPlaylistTracksFromBackend(playlistURL) {
  try {
    ...
    const response = await fetch(`https://spotify-clone-backend-empc.onrender.com/api/playlist-track-uri?playlistUrl=${encodedURL}`);
    const data = await response.json();
